...
| Qwen3-Coder-Next-80B | ✓ tested on vllm-metal but **not recommended** | llama.cpp (`qwennext-serve`) | Tool calling works (`hermes` parser, not `qwen3_coder` — see notes above), but the operational footprint is too punishing for daily use on a 64 GB Mac: 60 GB wired, ~1.8 GB Metal headroom, system pressure makes the whole machine sluggish. **Not wired into `~/.pi/agent/models.json`**, no `scripts/vllm-metal-serve` auto-pick — the empirical findings above are kept as a reference for future hardware (96 GB+ Mac) or a smaller quant if one appears. Upstream name is `Qwen3-Next-80B-A3B-Instruct` (no "Coder"), emits JSON-in-`<tool_call>` envelopes. |

**For everyday pi work** (revised 2026-05-14 after side-by-side use), **llama.cpp stays the default for Qwen3-Coder-30B** despite vllm-mlx/vllm-metal's +25% decode advantage. The MLX checkpoints we tested are all 4-bit; llama.cpp's `qwen-serve` runs Q5_K_M GGUF (higher precision). On real coding tasks the quality difference is felt — Q5_K_M produces noticeably better code than 4-bit MLX, and that matters more than raw tok/s for an interactive editing workflow. The MLX engines remain available for cases where decode throughput dominates (long-form generation, non-coding survey tasks). gpt-oss-20b stays on llama.cpp too (vllm-metal blockers, vllm-mlx tool-call gap). GLM-Air's vllm-mlx path works but prefill cost makes it impractical interactively — keep llama.cpp's `glmair-serve`.
...
| `mlx-community/gpt-oss-20b-MXFP4-Q4` | ✗ FAIL (two upstream blockers) | Stock vllm-metal 0.20.2: HTTP 500. `vllm_metal/metal_kernel_backend/attention_sdpa.py:592` reads `inner.scale` on `mlx_lm.models.gpt_oss.AttentionBlock`, which exposes `sm_scale` instead — `AttributeError`. The adapter already has getattr fallbacks for `n_heads`/`n_kv_heads` (line 401) but not `scale`. **Blocker 1** per [vllm-metal#212](https://github.com/vllm-project/vllm-metal/issues/212). Patched the two call sites locally (`getattr(inner, "scale", None) or inner.sm_scale`) — model then loads and generates at ~36–40 tok/s, but `tool-call-test` hangs: no structured `tool_calls` ever emitted, model runs away in the harmony analysis channel. This is **Blocker 2** from the same upstream issue: the Metal paged-attention kernel hardcodes `use_sinks=false` (`metal/paged_ops.cpp:193`) and `sliding_window=-1` (`attention_sdpa.py:139`); gpt-oss requires `use_sinks=true` and `sliding_window=128`. No client-side fix — both blockers are upstream kernel work. Status (gh-checked 2026-05-14): #212 still open; contributor `sshkhr` claimed a working M1 Max fix on 2026-04-20 with PR "in a couple days" but no PR has merged in the 24 days since. Keep `gptoss-serve` (llama.cpp) for gpt-oss-20b. |
| `mlx-community/GLM-4.5-Air-4bit` | ✗ FAIL (KV cache OOM, no fix) | Tested 2026-05-14 with `iogpu.wired_limit_mb=61440` and `VLLM_METAL_MEMORY_FRACTION=0.95`. Result: `model_memory=60.14 GB, overhead=0.99 GB, kv_budget=0.07 GB, num_blocks=23, max_tokens_cached=368`. The upfront budget barely passed (0.07 GB KV slack) but `_check_enough_kv_cache_memory` rejected the config — 23 blocks / 368 cacheable tokens is far below the 32K `--max-model-len` target. **GLM-4.5-Air-4bit MLX weights are ~60 GB**, much bigger than Qwen3-Next-80B-4bit's 44.84 GB; doesn't fit on a 64 GB Mac via vllm-metal at any wired-memory / fraction setting. Keep `glmair-serve` (llama.cpp) — llama.cpp doesn't do the upfront KV budget check and runs the same model successfully at ~22.9 tok/s (see GLM-4.5-Air on vllm-mlx results above for the comparison). |
| `mlx-community/Qwen3-Next-80B-A3B-Instruct-4bit` | ✓ PASS (with operational tuning) | **Parser**: `hermes`, **NOT** `qwen3_coder` — this is the regular `Qwen3-Next-80B-A3B-Instruct` (no "Coder" in the upstream name, despite our internal `local-qwen3-coder-next` alias). The Instruct variant emits `<tool_call>{"name":...,"arguments":{...}}</tool_call>` (JSON inside an envelope, the Hermes/Qwen2 format), not the XML format the Coder variant uses. With `--tool-call-parser qwen3_coder` the response landed correctly in `content` but `tool_calls` was empty `[]`; switching to `hermes` produces structured `tool_calls`. **Memory fit on 64 GB Mac is tight** — model weights are ~44.8 GB and vllm-metal's minimum KV reservation is ~19.78 GB (computed from model dims, not `--max-model-len`). Default `iogpu.wired_limit_mb=0` and `VLLM_METAL_MEMORY_FRACTION=0.9` shorts the KV budget by ~15.77 GB. Bumping `sudo sysctl iogpu.wired_limit_mb=61440` (60 GB) alone shrinks the gap to ~4.67 GB; you also need `VLLM_METAL_MEMORY_FRACTION=0.95` **and** `--max-num-seqs 1 --max-num-batched-tokens 2048` to make `kv_budget` go positive (~15 GB). Launch: `VLLM_METAL_MEMORY_FRACTION=0.95 vllm-metal-serve mlx-community/Qwen3-Next-80B-A3B-Instruct-4bit --max-model-len 32768 --max-num-seqs 1 --max-num-batched-tokens 8192`. **TTFT note**: the bare-minimum `--max-num-batched-tokens 2048` makes startup fit but produces painful TTFT on real pi sessions — chunked prefill of pi's system prompt + AGENTS.md (typically 5–20 K tokens) takes 3–10 prefill steps before first decode. Bumping to **8192** is the sweet spot — keeps `kv_budget` positive while shrinking TTFT ~4× on first turn. The runtime headroom after weights is ~1.8 GB available Metal memory; close other GPU-heavy apps (browsers, Slack/Electron) before launching or macOS will swap-thrash. This is the **absolute edge** of what fits on a 64 GB Mac — works reliably but with zero margin. The `vllm-metal-serve` wrapper now auto-picks `hermes` for any `*qwen3-next*` / `*qwen3*instruct*` model name. |
