...
| Qwen3-Coder-Next-80B | ✓ verified on vllm-metal | llama.cpp (`qwennext-serve`) for now | Wired-memory-cap + batch-shrink + `hermes` parser combo gets it running on a 64 GB Mac, but the operational footprint is heavy (60 GB wired, 2 GB OS headroom). For everyday use the llama.cpp path stays simpler. Note: the upstream name is `Qwen3-Next-80B-A3B-Instruct` (not "Coder"), and it emits JSON-in-`<tool_call>` envelopes — needs `--tool-call-parser hermes`, not `qwen3_coder`. Full details in the vllm-metal results table above. |
